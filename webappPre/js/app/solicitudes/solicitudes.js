...
app.controller('SolicitudesCtrl', ['$scope', '$state','$http', '$filter', '$modal', 'MyService', 'filterFilter', 'toaster','$timeout',  function($scope,  $state ,$http, $filter,$modal, MyService, filterFilter, toaster,$timeout) {
 $scope.nivel=MyService.data.nivel;
  ...
  $scope.consultarEnProceso = function(){
       $http.get('http://52.39.15.75:1345/solicitud/?estado=En proceso').then(function (resp) {   
  ...
  $scope.consultarEnProceso();
   $scope.consultarPendientes = function(){
    $http.get('http://52.39.15.75:1345/solicitud/?estado=Pendiente').then(function (resp) {
      $scope.solicitudes = resp.data.results;
      $scope.detallesSolicitud = null;
      $scope.lateral = false;   
    });
   ...
   $scope.consultarFinalizadas = function(){
    $http.get('http://52.39.15.75:1345/solicitud/?estado=Finalizada').then(function (resp) {
    $scope.solicitudes = resp.data.results;
    $scope.detallesSolicitud = null;
     $scope.lateral = true; 
    });
  ...
  $scope.selectItem = function(item){    
    ...
    $http.get('http://52.39.15.75:1345/consultor/?idEstablecimiento='+MyService.data.idEstablecimiento).then(function (resp) {
  ...
  $scope.deleteconsultor = function(consultor){
    $http.delete('http://52.39.15.75:1345/consultor/'+consultor.id , consultor)
  ...
  $scope.doneEditingConsultor = function(item){
    ...
    if (MyService.data.idenConsultor){
      $scope.pop4();
      $http.put('http://52.39.15.75:1345/consultor/'+MyService.data.idenConsultor , consultorAct)
    }
    else {
      $scope.pop3();;
      $http.post('http://52.39.15.75:1345/consultor/', consultorAct)
    }
    $http.get('http://52.39.15.75:1345/solicitud/').then(function (resp) {
      $scope.solicitudes = resp.data.results;
    });
    $http.get('http://52.39.15.75:1345/consultor/').then(function (resp) {
